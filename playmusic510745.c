...
void pluck(float buffer[], int size, float volume)
{
	int i;
	for(i = 0; i < size; i++)
	buffer[i] = volume * ((float)rand()/RAND_MAX - .5); //shift divide constant
}
...
//  double MAX = 1.0;
	 ...
	 if((input = fopen(argv[1],"r"))==NULL)
	 {
	  ...
	  num_samples = (((((tempo * next_note.time)) * 441)));// << 3) << 4) << 5) << 7) << 8; //shift multiply 1x 8x 16x 32x 128x 256x only thing that's been changed here so double check this
		...
		while(current_time < next_note.time)
		  {
			...
			for(j = 0; j < tracker; j++)
			{
				/*if (notes[active[j]][0] != 0) // should I if this when all notes (temp == frequency of string....)?
				{*/
